...
### CSV导出错误
**错误信息**: `{"error":"sequence item 4: expected str instance, NoneType found","success":false}`

**原因**: 某些门店的数据字段为 `None`（比如区域经理、城市等），而 `','.join()` 不能处理 `None` 类型

**修复**: 在CSV导出时，确保所有值都转换为字符串
```python
# 修复前
line = ','.join([
    str(store.get('store_id', '')),
    store.get('store_name', ''),  # 如果是None会报错
    ...
])
...
line = ','.join([
    ...
    str(store.get('store_name') or ''),  # None会被转换为空字符串
    ...
])
```
