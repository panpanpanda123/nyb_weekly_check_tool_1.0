...
def get_promo_regional_managers():
    """根据战区获取区域经理列表"""
    try:
        from shared.database_models import PromoParticipation
        session = get_db_session()
...
def search_promo():
    ...
    try:
        ...
        regional_manager = request.args.get('regional_manager', '').strip()
        store_search = request.args.get('store_search', '').strip()
        sort_by = request.args.get('sort_by', 'participation_rate').strip()  # 默认按活动参与度排序
        sort_order = request.args.get('sort_order', 'desc').strip()  # desc=降序, asc=升序
...
def export_promo():
    ...
    try:
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            worksheet.column_dimensions['C'].width = 10  # 战区
            worksheet.column_dimensions['D'].width = 12  # 区域经理
            worksheet.column_dimensions['E'].width = 12  # 订单量
            worksheet.column_dimensions['F'].width = 15  # 权益卡销量
            worksheet.column_dimensions['G'].width = 15  # 活动套餐销量
            worksheet.column_dimensions['H'].width = 15  # 活动参与度
            worksheet.column_dimensions['I'].width = 15  # 数据日期
